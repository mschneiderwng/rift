...
def ancestor(snapshot: Snapshot, source: Dataset, target: Dataset) -> Optional[Snapshot | Bookmark]:
    ...
    candidates = filter(lambda s: s.creation < snapshot.creation, source.snapshots() + source.bookmarks())
...
def send(snapshot: Snapshot, source: Dataset, target: Dataset, *, bwlimit: Optional[str] = None, dry_run: bool) -> None:
    ...
    if not target.exists():
        ...
        log.info(f"flux send (full) [{sizeof_fmt(stream.size())}] '{snapshot.fqn}' to '{source.fqn}'")
        return stream.to(target, bwlimit=bwlimit, dry_run=dry_run)

    if snapshot.guid in map(attrgetter("guid"), target.snapshots()):
        log.info(f"flux send '{snapshot.fqn}' to '{source.fqn}' skipped since snapshot already on target")
        return None

    elif (token := target.resume_token()) is not None:
        stream = source.send(token)
        log.info(f"flux send (resume) [{sizeof_fmt(stream.size())}] '{snapshot.fqn}' to '{source.fqn}'")
        ...
        log.info(f"flux send (incremental) [{sizeof_fmt(stream.size())}] '{snapshot.fqn}' to '{source.fqn}'")
...
def sync(source: Dataset, target: Dataset, *, bwlimit: Optional[str] = None, regex: str = ".*", dry_run: bool) -> None:
    """Send all snapshots from source to target which match the regex"""
    log = structlog.get_logger()
    log.info(f"flux sync newer snapshots from '{source.fqn}' to '{target.fqn}'")
...
def prune(dataset: Dataset, policy: dict[str, int], *, dry_run: bool) -> None:
    ...
    # for example, {"flux_.*_hourly": 24, "flux_.*_weekly": 7}
