...
from flux.datasets import Backend, Dataset, Stream, ancestor, prune, send, sync
from flux.snapshots import Bookmark, Snapshot
...
class InMemoryBackend(Backend):
    ...
    def snapshot(self, name: str) -> None:
        ...
        snapshot = Snapshot(
            fqn=self.path + "@" + name, guid="uuid:" + self.path + "@" + name, creation=str(next_creation())
        )
    ...
    def recv(self, stream: Stream, bwlimit: Optional[str], dry_run: bool) -> None:
        assert isinstance(stream, (FullInMemoryStream, IncrementalInMemoryStream, ResumingInMemoryStream))
...
def test_fqn_remote():
    assert_that(Dataset(InMemoryBackend("source/A", remote="user@host")).fqn, equal_to("user@host:source/A"))
...
def test_bookmark():
    ...
    source.bookmark("s2")
    assert_that(source.bookmarks(), equal_to((Bookmark(fqn="source/A#s2", guid="uuid:source/A@s2", creation="2"),)))
...
def test_sync_filtered():
    s1 = Snapshot(fqn="source/A@flux_s1", guid="uuid:source/A@flux_s1", creation="1")
    s2 = Snapshot(fqn="source/A@s2", guid="uuid:source/A@s2", creation="2")
    s3 = Snapshot(fqn="source/A@flux_s3", guid="uuid:source/A@flux_s3", creation="3")
    s4 = Snapshot(fqn="source/A@s3", guid="uuid:source/A@s4", creation="4")
    source = Dataset(InMemoryBackend("source/A", snapshots_data=[s1, s2, s3, s4]))
    target = Dataset(InMemoryBackend("target/backups/A", snapshots_data=[]))

    # sync newer from source to target
    sync(source, target, regex="flux_.*", dry_run=False)
    assert_that(target.snapshots(), contains_exactly(s1, s3))


def test_prune():
    s1 = Snapshot(fqn="source/A@flux_s1_weekly", guid="uuid:source/A@s1", creation="1")
    s2 = Snapshot(fqn="source/A@flux_s2_weekly", guid="uuid:source/A@s2", creation="2")
    s3 = Snapshot(fqn="source/A@flux_s3_daily", guid="uuid:source/A@s3", creation="3")
    s4 = Snapshot(fqn="source/A@flux_s4_monthly", guid="uuid:source/A@s4", creation="4")
    source = Dataset(InMemoryBackend("source/A", snapshots_data=[s1, s2, s3, s4]))
    policy = {"flux_.*_daily": 5, "flux_.*_weekly": 1, "flux_.*_monthly": 0}
