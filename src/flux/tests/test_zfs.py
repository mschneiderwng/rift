...
from flux.commands import NoSuchDatasetError, Runner
from flux.snapshots import Bookmark, Snapshot
from flux.zfs import ZfsBackend, ZfsStream
...
def test_snapshot_remote():
    ...
    dataset.snapshot("s2")
    assert_that(runner.recorded, equal_to([("ssh", "user@host", "--", "zfs", "snapshot", "source/A@s2")]))
...
def test_bookmark_remote():
    ...
    dataset.bookmark("s2")
    assert_that(
        runner.recorded, equal_to([("ssh", "user@host", "--", "zfs", "bookmark", "source/A@s2", "source/A#s2")])
    )
...
def test_snapshots():
    ...
    assert_that(
        runner.recorded,
        equal_to([("zfs", "list", "-pHt", "snapshot", "-o", "name,guid,creation", "source/A")]),
    )
...
def test_bookmarks():
    ...
    assert_that(
        runner.recorded,
        equal_to([("zfs", "list", "-pHt", "bookmark", "-o", "name,guid,creation", "source/A")]),
    )
...
def test_send_incremental_from_bookmark():
    ...
    assert_that(stream, equal_to(ZfsStream(("zfs", "send", "-w", "-i", "source/A#s1", "source/A@s2"), runner)))
...
def test_recv():
    ...
    assert_that(runner.recorded, equal_to([("zfs", "send", "..."), ("zfs", "receive", "-s", "-u", "source/A")]))
...
def test_recv_bwlimit():
    ...
    assert_that(
        runner.recorded,
        equal_to([("zfs", "send", "..."), ("mbuffer", "-m", "1M"), ("zfs", "receive", "-s", "-u", "source/A")]),
    )
...
def test_resume_token():
    ...
    assert_that(runner.recorded, equal_to([("zfs", "get", "-H", "-o", "value", "receive_resume_token", "source/A")]))
...
def test_stream_size():
    runner = TestRunner(
        returns="send from source/A@flux_2025-10-11_10:42:52_weekly to source/A@flux_2025-10-11_12:40:19_weekly estimated size is 624B\ntotal estimated size is 624"
    )
