...
import flux.datasets
from flux.commands import SystemRunner
from flux.datasets import Dataset
from flux.snapshots import Bookmark, Snapshot
from flux.zfs import ZfsBackend
...
def send(source, target, bwlimit, dry_run, verbose):
    ...
    with error_handler():
        ...
        return flux.datasets.send(snapshot, source, target, bwlimit=bwlimit, dry_run=dry_run)
...
def sync(source, target, regex, bwlimit, dry_run, verbose):
    ...
    with error_handler():
        ...
        flux.datasets.sync(source, target, regex=regex, bwlimit=bwlimit, dry_run=dry_run)
...
@click.option(
    ...
    help="Retention rule (e.g. '--keep 24 flux_.*_hourly --keep 4 flux_.*_weekly')",
)
@dry_run_option()
@verbose_option()
def prune(dataset, keep, dry_run, verbose):
    ...
    with error_handler():
        ...
        flux.datasets.prune(dataset=dataset, policy=policy, dry_run=dry_run)
